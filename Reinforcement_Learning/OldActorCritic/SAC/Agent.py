...
from ActorCriticFamily.common.NeuralNetworks import actor_network, critic_network, value_network

class Agent:
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=8,
                 environment=None, gamma=0.99, n_actions=2, buffer_size=1_000_000,
                 tau=0.005, batch_size=256, reward_scale=2):

        self.env = environment
        self.n_actions = environment.action_space.shape

        self.memory = ReplayBufferNP(buffer_size, input_dims, n_actions)
        self.batch_size = batch_size
        self.gamma = gamma
        self.tau = tau

        self.actor = ActorNetwork(n_actions=n_actions, name='actor', max_action=environment.action_space.high)
