...
def synth(frequency: float, duration: float = 10, sampling_rate: int = 44_100):
    """

    :param frequency: The frequency of the note to be played
    :param duration: Duration of the sound
    :param sampling_rate: Something
...
def keyboard(window, key, scancode, act, mods):
    ...
    if act == glfw.PRESS and key == glfw.KEY_LEFT:
        data.ctrl[kai[hand_in_use+str(key)]] -= 0.005
    ...
    if act == glfw.PRESS and key == glfw.KEY_KP_DECIMAL:
        data.ctrl[:23] = np.asarray([-0.2,   -0.25,   0.15,   1.4 ,   0.8 ,   0.2,    0.75,   0.45,   0.25,   0.6,
             0.5,    0. ,    0.1,    0.45,   0.55,   0. ,    0.55  , 0.   , -0.35,  -0.15,
            -0.07,   0.155 ,-0.015])

    if act == glfw.PRESS and key == glfw.KEY_KP_0:
        data.ctrl[:23] = main1_init

    if act == glfw.PRESS and key == glfw.KEY_KP_1:
        data.ctrl[:23] = main1_c5

    if act == glfw.PRESS and key == glfw.KEY_KP_2:
        data.ctrl[:23] = main1_b4

    if act == glfw.PRESS and key == glfw.KEY_KP_3:
        data.ctrl[:23] = main1_csharp5

    if act == glfw.PRESS and key == glfw.KEY_KP_4:
        data.ctrl[:23] = main1_gsharp4

    if act == glfw.PRESS and key == glfw.KEY_KP_5:
        data.ctrl[:23] = main1_dsharp5

    if act == glfw.PRESS and key == glfw.KEY_KP_6:
        data.ctrl[:23] = main2_init

    if act == glfw.PRESS and key == glfw.KEY_KP_7:
        data.ctrl[:23] = main2_c5

    if act == glfw.PRESS and key == glfw.KEY_KP_8:
        data.ctrl[:23] = main3_d5

    if act == glfw.PRESS and key == glfw.KEY_KP_9:
        data.ctrl[:23] = np.asarray([-0.2,   -0.25,   0.2,    1.4 ,   0.5 ,   0.2,    1.4,    0.8,    0.25,   0.45,
             0.5,    0. ,    0.1,    0.45,   0.55,   0. ,    0.55  , 0.   , -0.35,  -0.15,
            -0.07  , 0.155 ,-0.015])

    if act == glfw.PRESS and key == glfw.KEY_KP_DIVIDE:
        data.ctrl[:23] = np.asarray([-0.2,   -0.25,   0.2,    1.4 ,   0.85,   0.2,    1.4,    0.8,    0.3 ,   0.8,
             0.5,    0. ,    0.1,    0.45,   0.55,   0. ,    0.55  , 0.   , -0.35,  -0.15,
            -0.07  , 0.155 ,-0.015])

    if act == glfw.PRESS and key == glfw.KEY_KP_MULTIPLY:
        data.ctrl[:23] = np.asarray([-0.2,   -0.25,   0.2,    1.4 ,   0.85,   0.2,    1.4,    0.8,    0.3 ,   0.5,
             0.5,    0. ,    0.1,    0.45,   0.55,   0. ,    0.75  , 0.   , -0.35,  -0.15,
            -0.07  , 0.155 ,-0.015])

    if act == glfw.PRESS and key == glfw.KEY_KP_SUBTRACT:
        data.ctrl[:23] = np.asarray([-0.2,   -0.1,    0.2,    1.4 ,   0.7 ,   0.2,    1.4,    0.4,    0.3 ,   0.5,
             0.5,    0. ,    0.1,    0.45,   0.55,  -0.1,    0.4,    0. ,   -0.05,  -0.5,
            -0.07  , 0.155 , 0.   ])

    if act == glfw.PRESS and key == glfw.KEY_KP_ENTER:
        data.ctrl[:23] = np.asarray([-0.2,   -0.25,   0.2,    1.4 ,   0.85,   0.2,    1.4,    0.8,    0.3 ,   0.5,
             0.5,    0. ,    0.1,    0.45,   0.55,   0. ,    0.75  , 0.   , -0.35,  -0.15,
            -0.07  , 0.155 ,-0.015])

    if act == glfw.PRESS and key == glfw.KEY_KP_ADD:
        data.ctrl[:23] = np.asarray([-0.2,   -0.1,    0.2,    1.4 ,   0.7 ,   0.2,    1.4,    0.7,    0.3 ,   0.5,
             0.5,    0. ,    0.1,    0.45,   0.55,  -0.1,    0.95,   0. ,   -0.05,  -0.5,
            -0.07,   0.155,  0.   ])

    if hand_in_use == 'RIGHT_HAND ':
        if act == glfw.PRESS and key == glfw.KEY_1:
            data.ctrl[kai[hand_in_use+str(key)]] += 0.05
...
def play(song: callable):
    """
    Start playing the song given and also renders the hand in the simulator.
    :param song: A function of array sequencies of the hand's positions.
    """
    async def main():
        task1 = asyncio.create_task(simulation())
        task2 = asyncio.create_task(song(data, left_hand, right_hand))
